...
class ShapesManager:
    ...
    def add_circle(self,position: Position, color: str,radius:int=4):
        ...
        self.shapes[self.shapes_counter] =\
            Circle(shape_id ,position=position,color=color,radius=radius)
    ...
    def add_square(self,position: Position, color: str,height:int=8,width:int=8):
        ...
        shape_id =self.shapes_counter
        self.shapes[self.shapes_counter] = \
        Rectangle(shape_id,position=position,color=color,height=height,width=width)
        self.shapes_counter+=1
        return shape_id
